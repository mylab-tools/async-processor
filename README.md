...
* если в настройках указана `очередь-callback`, то API при изменении состояния запроса будет отправлять сообщение в эту очередь с соответствующим содержанием.
...
* `Callback` - определяет очередь для отправки уведомлений об изменении состояния запроса.
...
API асинхронного процессора поддерживает событийную модель взаимодействия. При изменении состояния запроса будет отправляться сообщение в очередь с соответствующим содежанием. 
...
Подробнее - [тут](./doc/callback-msg-schema.json).
